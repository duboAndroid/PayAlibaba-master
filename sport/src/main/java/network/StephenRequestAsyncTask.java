...
public class StephenRequestAsyncTask extends AsyncTask<String, Integer, String> {
    ...
    public StephenRequestAsyncTask(Context activity, Handler mainHandler, String requestWebUrl, int requestHttpCode, String requestHttpType, boolean showLoading) {//json
    ...
    public StephenRequestAsyncTask(Context activity, Handler mainHandler, String requestWebUrl, String requestHttpType, boolean showLoading, RequestCallback requestCallback) {//json(回调处理结果)
        this.isParamJson = true;
        this.activity = activity;
        this.mainHandler = mainHandler;
        this.requestWebUrl = requestWebUrl;
    ...
    public StephenRequestAsyncTask(Context activity, Handler mainHandler, String requestWebUrl, int requestHttpCode, String requestHttpType, boolean showLoading, List<NameValuePair> nameValuePairParams, Map<String, File> uploadFileMap) {//form
        this.isParamJson = false;
        this.activity = activity;
        this.mainHandler = mainHandler;
        this.requestWebUrl = requestWebUrl;
        ...
        try {
            if(isParamJson){//json
                if(requestHttpType.equals(VickyConfig.RequestType_Get)){
                    ...
                    //.LogD(requestHttpCode+"网络请求Url==>"+requestWebUrl);
                    //.LogD(requestHttpCode+"网络请求参数==>Key:"+requestParams[0]+"\n==>FilePath:"+requestParams[1]);
                ...
                if(requestHttpType.equals(VickyConfig.RequestType_Get)){
                    //.LogD(requestHttpCode+"网络请求Url及参数==>"+requestWebUrl);
                    return NetworkUtil.sendGetRequest(activity,requestWebUrl);
                }else if(requestHttpType.equals(VickyConfig.RequestType_Post)){
                    //.LogD(requestHttpCode+"网络请求Url==>"+requestWebUrl);
                    //.LogD(requestHttpCode+"网络请求参数==>"+JsonUtil.toJson(nameValuePairParams));
                    return NetworkUtil.sendPostRequest(activity,requestWebUrl,nameValuePairParams);
                }else if(requestHttpType.equals(VickyConfig.RequestType_Upload)){
                    //.LogD(requestHttpCode+"网络请求Url==>"+requestWebUrl);
                    //.LogD(requestHttpCode+"网络请求参数==>Params:"+JsonUtil.toJson(JsonUtil.toJson(nameValuePairParams))+"\n==>Files:"+JsonUtil.toJson(uploadFileMap));
    ...
    protected void onPostExecute(String result) {
        //.LogD(requestHttpCode+"网络请求结果==>"+(HtmlRegexpUtil.hasSpecialChars(result) ? HtmlRegexpUtil.filterHtml(result) : result));
